...
import com.wind.common.utils.validate.ValidateUtils;
 ...
 * 
 */
public class DateTool extends org.apache.commons.lang.time.DateUtils {
	...
	/**
	 *DATE Format的格式： yyyy-MM-dd
	 */
	public static String DATE_FORMAT_DATEONLY = "yyyy-MM-dd"; // 年/月/日
	
	/**
	 *DATE Format的格式： yyyy-MM-dd HH:mm:ss
	 */
	public static String DATE_FORMAT_DATETIME = "yyyy-MM-dd HH:mm:ss"; // 年/月/日
	public static SimpleDateFormat sdfDateTime = new SimpleDateFormat(DateTool.DATE_FORMAT_DATETIME);
	// Global SimpleDateFormat object
	public static SimpleDateFormat sdfDateOnly = new SimpleDateFormat(DateTool.DATE_FORMAT_DATEONLY);
	...
	public static final SimpleDateFormat  STR_NAME_FILE_DATE_FORMAT =  new SimpleDateFormat("yyyyMMdd_HHmmss");
	...
	public static String getNameFileCurrentDate() {
		Date date = new Date();
		return STR_NAME_FILE_DATE_FORMAT.format(date);
	}
	...
	public static Timestamp toSqlTimestamp(String sDate) {
		...
		if (sDate.length() != DateTool.DATE_FORMAT_DATEONLY.length()
				&&sDate.length() != DateTool.DATE_FORMAT_DATETIME.length()) {
			return null;
		}
		return toSqlTimestamp(sDate,
				sDate.length() == DateTool.DATE_FORMAT_DATEONLY.length()
				?DateTool.DATE_FORMAT_DATEONLY
				:DateTool.DATE_FORMAT_DATETIME);
	...
	public static Timestamp toSqlTimestamp(String sDate, String sFmt) {
		...
		if (sFmt.equals(DateTool.DATE_FORMAT_DATETIME)) {
			temp = sDate.replace('/', '-');
			temp = temp + ".000000000";
		} else if (sFmt.equals(DateTool.DATE_FORMAT_DATEONLY)) {
			temp = sDate.replace('/', '-');
	...
	public static String getSysDateTimeString() {
		return toString(new Date(System.currentTimeMillis()), DateTool.sdfDateTime);
	}
	...
	public static String toSqlTimestampString2(Timestamp dt) {
		...
		String temp = toSqlTimestampString(dt, DateTool.DATE_FORMAT_DATETIME);
	...
	public static String toSqlTimestampString(Timestamp dt, String sFmt) {
		...
		temp = dt.toString();
		if (sFmt.equals(DateTool.DATE_FORMAT_DATETIME) || // "YYYY/MM/DD
				// HH24:MI:SS"
				sFmt.equals(DateTool.DATE_FORMAT_DATEONLY)) { // YYYY/MM/DD
	...
	public static String timestampToStringYMD(Timestamp timestamp) {
		SimpleDateFormat sdf = new SimpleDateFormat(DateTool.DATE_FORMAT_DATEONLY);
	...
	public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
		...
		Date endDate = null;
		try {
			startDate = DateTool.parseDate(startDateStr, DateTool.DATE_FORMAT_DATEONLY);
			endDate = DateTool.parseDate(endDateStr, DateTool.DATE_FORMAT_DATEONLY);
		} catch (ParseException e) {
			logger.error(e);
			return false;
		}
	...
	public static Date getMinTime(Date dt) {
		Date dt1 = null;
		dt1 = DateTool.getDateByStr(DateTool.formatDate(dt, "yyyy-MM-dd"));
		return dt1;
	}
	...
	public static List<String> getLastDays(int countDay) {
		...
		for (int i = 0; i < countDay; i++) {
			listDate.add(DateTool.getReqDateyyyyMMdd(DateTool.getDate(-i)));
		}
		return listDate;
	}
	...
	public static void main(String[] args) {
		Date date3 = DateTool.addMinutes(new Date(), 5);
		System.out.println(new Date().after(DateTool.addMinute(new Date(), -10)));
		Date date1 = DateTool.addMinutes(new Date(), -1);
		Date date2 = DateTool.addMinutes(date1, 5);
		System.out.println(date1);
		System.out.println(date2);
		System.out.println(DateTool.isBetween(new Date(), date1, date2, DateTool.COMP_MODEL_TIME));
